...
export default function TambahFAQ() {
  ...
  const [form, setForm] = useState({ pertanyaan: "", jawaban: "", urutan: 0 });
    ...
    <main className="min-h-screen bg-slate-900 p-8">
      <div className="max-w-2xl mx-auto">
        <div className="flex justify-between items-center mb-8">
          <h1 className="text-2xl font-bold text-white">Tambah FAQ</h1>
        ...
        <div className="bg-slate-800 rounded-2xl p-6 border border-slate-700 flex flex-col gap-4">
          <div>
            ...
            <input name="pertanyaan" value={form.pertanyaan} onChange={handleChange} className={inputClass} placeholder="Apa itu kredit syariah?" />
          </div>
          <div>
            <label className={labelClass}>Jawaban</label>
            <textarea name="jawaban" value={form.jawaban} onChange={handleChange} className={inputClass} rows={6} placeholder="Tulis jawaban lengkap di sini..." />
          </div>
          <div>
            <label className={labelClass}>Urutan Tampil</label>
            <input name="urutan" type="number" value={form.urutan} onChange={handleChange} className={inputClass} placeholder="0" />
          </div>
